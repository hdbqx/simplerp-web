...
function appItemTpl(app) {
  ...
  const cmd = Array.isArray(app.command) ? app.command.join(" ") : (app.command || "");
  meta.innerHTML = `<strong>${app.name}</strong><div class="cmd">${cmd}</div>`;
...
function renderStatus(status) {
  if (els.configPath) {
    ...
    els.configPath.title = status?.configPath || "";
  }
...
async function init() {
  ...
  window.managerApi.onLog((payload) => appendLog(payload));

  els.btnLoadConfig?.addEventListener("click", async () => {
    const s = await window.managerApi.loadConfig();
    renderStatus(s);
  });
  els.btnOpenFolder?.addEventListener("click", () => window.managerApi.openFolder());
  els.btnCloudStart?.addEventListener("click", () => window.managerApi.startCloudflared());
  els.btnCloudStop?.addEventListener("click", () => window.managerApi.stopCloudflared());
  els.btnCloudRestart?.addEventListener("click", () => window.managerApi.restartCloudflared());
  els.btnStartSelected?.addEventListener("click", () => window.managerApi.startSelected());
  els.btnStopSelected?.addEventListener("click", () => window.managerApi.stopSelected());
  els.btnStartAll?.addEventListener("click", () => window.managerApi.startAll());
  els.btnStopAll?.addEventListener("click", () => window.managerApi.stopAll());
