...
function App() {
  ...
  const handleSend = async () => {
    ...
    if (viewMode === 'char') handleTriggerAI(characters.find(c=>c.id===selectedCharId)!, text);
  };

  const handleTriggerAI = async (char: Character, textOverride?: string) => {
    ...
    const groupCtx = viewMode === 'group' ? {
      name: groups.find(g => g.id === selectedGroupId)?.name || "群聊",
    ...
    try {
      ...
      await api.messages.add({ 
        ...
        group_id: viewMode === 'group' ? selectedGroupId : undefined, timestamp: tempTs 
      });
  ...
  const handleGenImage = async () => {
    ...
    const res = await fetch(`${settings.sd_url.replace(/\/$/, '')}/sdapi/v1/txt2img`, {
      ...
      body: JSON.stringify({ prompt: genPrompt, steps: 20, width: 512, height: 768 })
    });
  ...
  const Sidebar = () => (
    <div className="flex flex-col h-full bg-base-200 w-80 p-4 border-r border-base-content/10 shadow-xl overflow-hidden">
      ...
      <div className="tabs tabs-boxed mb-6">
        <button className={`tab flex-1 ${viewMode === 'char' ? 'tab-active' : ''}`} onClick={() => setViewMode('char')}>私聊</button>
      ...
      <div className="mt-4 pt-4 border-t border-base-content/10">
        <button className="btn btn-ghost btn-sm btn-block justify-start" onClick={() => setShowSettings(true)}><SettingsIcon size={16} /> 设置</button>
      </div>
    </div>
  );

  if (isLoading) return <div className="h-screen flex items-center justify-center"><span className="loading loading-spinner loading-lg"></span></div>;
  ...
  return (
    <div className="drawer md:drawer-open h-[100dvh] w-full bg-base-100">
      ...
      <div className="drawer-content flex flex-col h-full overflow-hidden relative">
        {/* Navbar */}
        <div className="navbar bg-base-100/90 backdrop-blur-md border-b border-base-300 px-4 sticky top-0 z-20">
          ...
          <div className="flex-1 font-bold text-lg truncate px-2">{viewMode==='char'?characters.find(c=>c.id===selectedCharId)?.name:groups.find(g=>g.id===selectedGroupId)?.name || "请选择"}</div>
          <div className="flex-none gap-1">
            ...
            {viewMode === 'char' && selectedCharId && (
              <>
                <button className="btn btn-sm btn-ghost text-info" onClick={async ()=>{setIsSummarizing(true); const s=await new LLMClient().summarize(messages, settings!); api.characters.update(selectedCharId, {summary:s}).then(loadBase); setIsSummarizing(false);}} disabled={isSummarizing}><BookOpen size={18}/></button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
              ...
              <div key={idx} className={`chat ${isUser ? 'chat-end' : 'chat-start'} group`}>
                <div className="chat-header opacity-50 text-[10px] mb-1 flex items-center gap-2">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                        <button className="hover:text-primary" onClick={()=>{setEditingMsgId(m.id!); setEditContent(m.content)}}><Pencil size={10}/></button>
                        <button className="hover:text-error" onClick={async ()=>{if(confirm("删?")) {await api.messages.delete(m.id!); setMessages(messages.filter(msg=>msg.id!==m.id))}}}><Trash2 size={10}/></button>
                    </div>
                </div>
                {m.image ? <div className="chat-bubble p-1 bg-base-200 border-base-300 shadow-xl"><img src={m.image} className="max-w-xs md:max-w-md rounded-lg"/></div> : (
                  <div className={`chat-bubble shadow-lg border ${isUser ? 'chat-bubble-primary border-primary' : 'bg-base-200 text-base-content border-base-300'}`}>
                    {editingMsgId === m.id ? (
                        <div className="flex flex-col gap-2"><textarea className="textarea textarea-bordered textarea-sm w-full bg-base-100" value={editContent} onChange={e=>setEditContent(e.target.value)}/><div className="flex justify-end gap-1"><button className="btn btn-xs" onClick={()=>setEditingMsgId(null)}>取消</button><button className="btn btn-xs btn-primary" onClick={async ()=>{await api.messages.update(m.id!, editContent); setMessages(messages.map(msg=>msg.id===m.id?{...msg, content:editContent}:msg)); setEditingMsgId(null)}}>保存</button></div></div>
          ...
          {isTyping && <div className="chat chat-start opacity-50 animate-pulse"><div className="chat-bubble">正在思考...</div></div>}
          <div ref={bottomRef} className="h-20" />
        </div>

        {/* Input */}
        <div className="p-4 bg-base-100/90 border-t border-base-300 sticky bottom-0 z-20">
          <div className="max-w-4xl mx-auto flex flex-col gap-2">
            {viewMode === 'group' && selectedGroupId && (
              <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                {characters.filter(c => groupMemberIds.includes(c.id!)).map(m => (
                  <button key={m.id} onClick={() => handleTriggerAI(m)} disabled={isTyping} className="btn btn-xs btn-outline btn-secondary whitespace-nowrap rounded-full">@{m.name}</button>
                ))}
              </div>
            )}
      ...
      {showGroupEdit && selectedGroupId && (
          <div className="modal modal-open">
              <div className="modal-box max-w-2xl h-[85vh] flex flex-col p-0 overflow-hidden">
                  <div className="p-6 border-b flex justify-between items-center bg-base-200"><h3 className="font-bold text-xl flex items-center gap-2"><Users/> 剧场管理</h3><button className="btn btn-sm btn-circle btn-ghost" onClick={()=>setShowGroupEdit(false)}><X/></button></div>
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                      ...
                      <div className="form-control"><label className="label font-bold">剧场大背景</label><textarea className="textarea textarea-bordered h-48 font-mono text-sm" value={groups.find(g=>g.id===selectedGroupId)?.description} onChange={e=>setGroups(groups.map(g=>g.id===selectedGroupId?{...g, description:e.target.value}:g))} placeholder="设定公共环境..."/></div>
                      <div className="space-y-4">
                          <label className="label font-bold text-primary">勾选剧场成员</label>
                  ...
                  <div className="p-6 border-t bg-base-200 flex justify-end gap-2">
                      <button className="btn btn-primary btn-block md:w-auto" onClick={async ()=>{const g=groups.find(grp=>grp.id===selectedGroupId); if(g){await api.groups.update(selectedGroupId, {...g, memberIds: groupMemberIds}); setShowGroupEdit(false); alert("保存成功")}}}>保存设定</button>
                  </div>
              </div>
          </div>
      )}
      ...
      {showCharEdit && selectedCharId && (
          <div className="modal modal-open">
              <div className="modal-box max-w-2xl h-[85vh] flex flex-col p-0">
                  <div className="p-6 border-b flex justify-between items-center bg-base-200"><h3 className="font-bold text-lg">角色编辑</h3><button className="btn btn-sm btn-circle btn-ghost" onClick={()=>setShowCharEdit(false)}><X/></button></div>
                  <div className="flex-1 overflow-y-auto p-6 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input className="input input-bordered" placeholder="独立模型 ID" value={characters.find(c=>c.id===selectedCharId)?.model_id || ''} onChange={e=>setCharacters(characters.map(c=>c.id===selectedCharId?{...c, model_id:e.target.value}:c))} />
                        <input className="input input-bordered" placeholder="独立 API Base" value={characters.find(c=>c.id===selectedCharId)?.api_base_override || ''} onChange={e=>setCharacters(characters.map(c=>c.id===selectedCharId?{...c, api_base_override:e.target.value}:c))} />
                      </div>
                      ...
                      <textarea className="textarea textarea-bordered w-full h-48 font-mono text-xs" placeholder="人格指令" value={characters.find(c=>c.id===selectedCharId)?.description} onChange={e=>setCharacters(characters.map(c=>c.id===selectedCharId?{...c, description:e.target.value}:c))} />
                      <textarea className="textarea textarea-bordered w-full h-24 font-mono text-xs" placeholder="长期记忆" value={characters.find(c=>c.id===selectedCharId)?.summary} onChange={e=>setCharacters(characters.map(c=>c.id===selectedCharId?{...c, summary:e.target.value}:c))} />
                  </div>
      ...
      {showGenModal && (
          <div className="modal modal-open">
              <div className="modal-box"><h3 className="font-bold text-lg mb-4">生成图片</h3><textarea className="textarea textarea-bordered w-full h-32" value={genPrompt} onChange={e=>setGenPrompt(e.target.value)} /><div className="modal-action"><button className="btn btn-primary" onClick={handleGenImage}>生成</button><button className="btn" onClick={()=>setShowGenModal(false)}>取消</button></div></div>
          </div>
      )}

      {showLorebook && selectedCharId && (
          <div className="modal modal-open">
              <div className="modal-box max-w-2xl h-[70vh] flex flex-col"><h3 className="font-bold text-lg mb-4 flex items-center gap-2"><Book/> 世界书</h3><div className="flex-1 overflow-y-auto space-y-3">
                  {lorebookEntries.map(e => (
                      <div key={e.id} className="collapse collapse-arrow bg-base-200"><input type="checkbox"/><div className="collapse-title font-bold">{e.keywords}</div><div className="collapse-content space-y-2"><textarea className="textarea textarea-bordered w-full h-24 text-xs" defaultValue={e.content} onBlur={(evt)=>api.lorebook.update(e.id!, {content: evt.target.value})} /><div className="flex gap-2"><input className="input input-bordered input-sm flex-1" defaultValue={e.keywords} onBlur={(evt)=>api.lorebook.update(e.id!, {keywords: evt.target.value})} /><button className="btn btn-sm btn-error" onClick={()=>api.lorebook.delete(e.id!).then(()=>api.lorebook.list(selectedCharId).then(setLorebookEntries))}>删除</button></div></div></div>
                  ))}
              ...
              </div><div className="modal-action"><button className="btn" onClick={()=>setShowLorebook(false)}>关闭</button></div></div>
          </div>
      )}

      {showSettings && settings && (
          <div className="modal modal-open">
              <div className="modal-box"><h3 className="font-bold text-lg mb-4">系统设置</h3><div className="space-y-4"><input className="input input-bordered w-full" placeholder="API Base" value={settings.api_base} onChange={e=>setSettings({...settings, api_base:e.target.value})} /><input className="input input-bordered w-full" placeholder="API Key" type="password" value={settings.api_key} onChange={e=>setSettings({...settings, api_key:e.target.value})} /><textarea className="textarea textarea-bordered w-full" placeholder="Model List" value={settings.model_list} onChange={e=>setSettings({...settings, model_list:e.target.value})} /><input className="input input-bordered w-full" placeholder="SD URL" value={settings.sd_url} onChange={e=>setSettings({...settings, sd_url:e.target.value})} /></div><div className="modal-action"><button className="btn btn-primary" onClick={()=>{api.settings.update(settings); setShowSettings(false)}}>保存</button><button className="btn" onClick={()=>setShowSettings(false)}>取消</button></div></div>
          </div>
      )}
    </div>
  );
