...
function App() {
  ...
  const loadData = async () => {
    try {
        ...
        setCharacters(c); setGroups(g); setSettings(s);
      ...
      <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">
        {viewMode === 'char' ? characters.map(c => (
          <div key={c.id} onClick={() => { setSelectedCharId(c.id); setMobileMenuOpen(false); }} className={`p-3 rounded-xl cursor-pointer flex justify-between items-center group ${selectedCharId === c.id ? 'bg-primary text-primary-content shadow-lg' : 'hover:bg-base-300'}`}>
            ...
            <Trash2 size={14} className="opacity-0 group-hover:opacity-100 hover:text-error" onClick={(e) => { e.stopPropagation(); if(confirm("删除角色及其私聊历史？")) api.characters.delete(c.id!).then(() => loadData()); }} />
          </div>
        )) : groups.map(g => (
          <div key={g.id} onClick={() => { setSelectedGroupId(g.id); setMobileMenuOpen(false); }} className={`p-3 rounded-xl cursor-pointer flex justify-between items-center group ${selectedGroupId === g.id ? 'bg-secondary text-secondary-content shadow-lg' : 'hover:bg-base-300'}`}>
            ...
            <Trash2 size={14} className="opacity-0 group-hover:opacity-100 hover:text-error" onClick={(e) => { e.stopPropagation(); if(confirm("删除剧场？历史消息将同步删除。")) api.groups.delete(g.id!).then(() => loadData()); }} />
          </div>
        ))}
        <button className="btn btn-outline btn-sm btn-block mt-4 border-dashed" onClick={async () => { const n = prompt("输入名称:"); if(n) { viewMode==='char'? await api.characters.add({name:n, description:"", first_message:"你好"}): await api.groups.add({name:n, description:""}); loadData(); } }}><Plus size={16} /> 新建</button>
      </div>
    ...
    <div className="drawer md:drawer-open h-[100dvh] w-full bg-base-100 overflow-hidden text-base-content">
      ...
      <div className="drawer-content flex flex-col h-full overflow-hidden relative">
        ...
        <div className="navbar bg-base-100 border-b border-base-300 px-4 sticky top-0 z-20">
          ...
          <div className="flex-1 font-bold truncate px-2">{viewMode==='char'?characters.find(c=>c.id===selectedCharId)?.name:groups.find(g=>g.id===selectedGroupId)?.name || "请选择角色或剧场"}</div>
              ...
              <div className="modal-box max-w-3xl h-[85vh] flex flex-col p-0 overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6">
                      ...
                      <div className="space-y-4">
                          <label className="label font-bold text-primary">管理剧员 (勾选即加入)</label>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                              {characters.map(c => (
                                  <label key={c.id} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${groupMemberIds.includes(c.id!) ? 'border-primary bg-primary/10' : 'border-base-300 hover:border-base-content/10'}`}>
                  ...
                  <div className="p-6 border-t bg-base-200 flex justify-end gap-2"><button className="btn btn-primary btn-block md:w-auto" onClick={async ()=>{const g=groups.find(grp=>grp.id===selectedGroupId); if(g){await api.groups.update(selectedGroupId, {...g, memberIds: groupMemberIds}); setShowGroupEdit(false); alert("保存成功")}}}>保存剧场配置</button></div>
              </div>
          </div>
      )}
              ...
              <div className="modal-box max-w-2xl h-[85vh] flex flex-col p-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="form-control"><label className="label font-bold text-xs">独立 API 地址</label><input className="input input-bordered" placeholder="默认使用系统设置" value={characters.find(c=>c.id===selectedCharId)?.api_base_override || ''} onChange={e=>setCharacters(characters.map(c=>c.id===selectedCharId?{...c, api_base_override:e.target.value}:c))} /></div>
                      </div>
                  ...
                  <div className="p-4 border-t bg-base-200 flex justify-end"><button className="btn btn-primary" onClick={async ()=>{await api.characters.update(selectedCharId, characters.find(c=>c.id===selectedCharId)!); setShowCharEdit(false); loadData()}}>保存角色设定</button></div>
              </div>
          </div>
      )}
      ...
      {showSettings && settings && (
          <div className="modal modal-open">
              <div className="modal-box"><h3 className="font-bold text-lg mb-4">系统设置</h3><div className="space-y-4">
                  ...
                  <button className="btn btn-error btn-outline btn-block btn-sm" onClick={()=>{if(confirm("清理所有图片以释放数据库空间？")) api.messages.clearAllImages().then(()=>alert("清理完成"))}}><HardDrive size={16}/> 清理所有生成图片</button>
              </div><div className="modal-action"><button className="btn btn-primary btn-block" onClick={()=>{api.settings.update(settings).then(() => {setShowSettings(false); loadData();})}}>保存并应用</button></div></div>
          </div>
      )}
      ...
      {showLorebook && selectedCharId && (
          <div className="modal modal-open">
              <div className="modal-box max-w-2xl h-[70vh] flex flex-col p-0">
                  <div className="p-4 border-b flex justify-between bg-base-200 font-bold">世界书<button className="btn btn-sm btn-circle btn-ghost" onClick={()=>setShowLorebook(false)}><X/></button></div>
