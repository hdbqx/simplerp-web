...
export class LLMClient {
  ...
  async *chatStream(
    ...
    const messages = history.slice(-20).map(m => {
      ...
      if (groupCtx) {
        const sender = groupCtx.members.find(c => c.id === m.char_id);
  ...
  async summarize(history: Message[], settings: Settings): Promise<string> {
    ...
    const res = await this.client.chat.completions.create({
      model: model,
      messages: [{ role: 'system', content: "总结上述对话事实，保留性格变化和重要事件。" }, { role: 'user', content: history.map(m=>m.content).slice(-30).join('\n') }]
    });
